...
def _detect_backend() -> Optional[str]:
    ...
    try:
        ...
        if available_langs.size == 0:
            log.warning("系统未安装任何 OCR 语言包")
            return None

        # 尝试中文或英文
        lang_codes = ["zh-Hans", "zh-CN", "en", "en-US"]
        for code in lang_codes:
            try:
                lang = Language(code)
                if OcrEngine.is_language_supported(lang):
                    _available_backend = "windows"
                    log.info("OCR 后端: Windows 内置 OCR (%s)", code)
                    return _available_backend
            except Exception:
                continue
        ...
        if available_langs.size > 0:
            ...
            _available_backend = "windows"
            log.info("OCR 后端: Windows 内置 OCR (%s)", first_lang.language_tag)
            return _available_backend

    except ImportError as e:
        log.debug("winsdk 未安装: %s", e)
    except Exception as e:
        log.debug("Windows 内置 OCR 初始化失败: %s", e)
